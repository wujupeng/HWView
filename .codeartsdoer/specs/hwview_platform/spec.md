...
| 行号 | 行名称 | 数据来源类别 | 颜色编码 |
|------|--------|-------------|---------|
| 1 | 目标数量老线 | 人工录入/计划导入 | 绿色背景 |
| 2 | 实际完成数量（老线白班） | 人工录入/MES导入 | 默认 |
| 3 | 实际完成数量（老线夜班） | 人工录入/MES导入 | 默认 |
| 4 | 实际完成数量（新线白班） | 人工录入/MES导入 | 默认 |
| 5 | 实际完成数量（新线夜班） | 人工录入/MES导入 | 默认 |
| 6 | 合计（新老线实际完成数量） | 自动计算（行2+3+4+5） | 藍色背景 |
| 7 | 成品入库数量 | 人工录入/MES导入 | 默认 |
| 8 | 出货数量 | 人工录入/ERP导入 | 默认 |
| 9 | 成品库存量 | 自动计算（累计入库-累计出货） | 默认 |
| 10 | 产线成品剩余量 | 人工录入 | 默认 |

1. **行顺序规则**：报表数据行**必须**严格按上表顺序排列，**禁止**调整顺序。
   - 验收条件：[报表生成] → [行顺序为 目标老线→老线白班→老线夜班→新线白班→新线夜班→合计→入库→出货→库存→产线剩余]
2. **颜色编码规则**：目标/计划数据行（行1）**必须**绿色背景；合计数据行（行6）**必须**蓝色背景；节假日列**必须**黄色背景。
   - 验收条件：[报表导出] → [行1 绿色、行6 藍色、节假日列黄色]
...
2. **装箱数歧义待裁决规则**：§18.2.3 的"60 只/箱"与本节"30 件/箱"存在歧义，**必须**由 PM 裁决当前产线适用哪个 units_per_carton，裁决前**禁止**硬编码任一数值。
   - 验收条件：[PM 未裁决 units_per_carton 取值] → [配置项以占位符 `<UNITS_PER_CARTON>` 表示，不出现 30 或 60 硬编码]
3. **实际完成数据来源修正规则（R2-AMENDMENT）**：报表行2-行5（实际完成数量）的数据**必须**来源于 actual_quantity（经箱数换算后的实际件数），**禁止**直接使用 carton_count（整箱数）原值作为实际完成数量。
   - 验收条件：[报表行2-行5 数据来源审查] → [来源为 actual_quantity，非 carton_count 原值 17]
...
2. **合计行计算规则**：行6"合计（新老线实际完成数量）"**必须**等于行2+行3+行4+行5，按日列分别求和，**禁止**人工录入合计值。
   - 验收条件：[行2/3/4/5 某日录入 100/200/300/400] → [行6 该日自动显示 1000]
3. **库存计算规则**：行9"成品库存量"**必须**等于累计入库（行7 累计）减累计出货（行8 累计），**禁止**人工录入库存值。
   - 验收条件：[累计入库=1980，累计出货=1680] → [行9 自动显示 300]
4. **实际完成合计公式约束（R2-AMENDMENT）**：行6"合计（新老线实际完成数量）"的计算公式**必须**为：

   ```
   实际完成合计 = 行2（老线白班实际）+ 行3（老线夜班实际）+ 行4（新线白班实际）+ 行5（新线夜班实际）
   ```

   **禁止**将行1"目标数量老线"或任何目标数量行加入实际完成合计。
   - 验收条件：[行1 目标=2000，行2/3/4/5 实际=500/500/500/500] → [行6 合计=2000（实际完成之和），而非 4000（含目标）]
5. **实际完成合计与目标分离约束（R2-AMENDMENT）**：目标数量（行1）与实际完成合计（行6）**必须**在报表中分属不同行，**禁止**合并或相加。
   - 验收条件：[报表结构审查] → [行1 与行6 独立呈现，行6 计算不包含行1]
...
### **18.7.4 CartonSpecification（装箱规格配置，R2-AMENDMENT 新增）**

> 本对象对应 §16.4 R2-AMENDMENT 的数据模型影响，定义箱数/散件/实际件数的计算规格。每箱标准装箱数必须通过本配置对象管理，禁止硬编码。

1. **产线标识 line_id**：必填，关联 §6.1 Production Line，标识本装箱规格适用的产线
2. **产品编码 product_code**：必填，标识本装箱规格适用的产品型号
3. **每箱标准装箱数 units_per_carton**：必填，正整数，单位"件/箱"，**禁止**硬编码于源代码，**必须**通过本配置对象管理
4. **生效日期 effective_from**：必填，格式 YYYY-MM-DD，标识本规格生效起始日期
...
### **18.7.5 CartonBasedActualQuantity（基于箱数的实际产出，R2-AMENDMENT 新增）**

> 本对象对应 §16.4 R2-AMENDMENT 的实际产出计算模型，用于从箱数+散件推导实际件数，作为报表行2-行5（实际完成数量）的数据来源。

1. **日期**：必填，格式 YYYY-MM-DD，唯一标识组成部分
2. **产线标识 line_id**：必填，关联 §6.1 Production Line
3. **班次**：必填，取值 {老线白班, 老线夜班, 新线白班, 新线夜班}，对应 §18.2.2 行2-行5
4. **整箱数 carton_count**：必填，非负整数，来源于源系统 quantity 字段（R2-AMENDMENT 修正语义）或人工录入
5. **每箱标准装箱数 units_per_carton**：必填，非负整数，**必须**来源于 §18.7.4 CartonSpecification 配置，**禁止**硬编码
...
9. **与报表行对应约束**：本对象的 actual_quantity **必须**作为 §18.2.2 行2-行5（实际完成数量）的数据来源，**禁止**直接使用 carton_count 作为报表实际完成数量
   - 验收条件：[报表行2-行5 数据来源审查] → [来源为 actual_quantity，非 carton_count 原值]
...
| EV1-R4 R2-AMENDMENT 合计公式修正 | 行6 实际完成合计不包含行1 目标数量 | 待验收 |
| EV1-R4 R2-AMENDMENT 装箱数歧义裁决 | PM 已裁决当前产线 units_per_carton 取值（30 vs 60） | 待裁决 |
